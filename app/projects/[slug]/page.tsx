...
export async function generateStaticParams() {
  ...
  return items.filter(item => item.type === 'project').map((item) => ({
    slug: item.slug,
  }));
}
...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!itemData || itemData.type !== 'project') {
    notFound();
  }

  return (
    <article className="section">
      <Link href="/projects" style={{ color: "var(--accent-color)", fontWeight: 500, display: "inline-block", marginBottom: "var(--padding-even)" }}>
